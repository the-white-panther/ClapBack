...
import { buildPrompt, getModelForTone } from '../services/prompt.js';
...
analyzeRouter.post('/', validateAnalyzeRequest, async (c) => {
  ...
  const messages = buildPrompt(body.chatContext, body.tone, body.customTone);
  const model = getModelForTone(body.tone);

  try {
    const rawResponse = await callOpenRouter(messages, model);
